...
* Technical corrections
  * FHIR-58774: Corrected the cardinality of `BodyStructure.extension:includedStructure` in the R4 profile from `0..*` to `1..*`. The cross-version extension defines its own root as `1..*`, derived from `BodyStructure.includedStructure 1..*` in R5, so the profile contradicted the extension it uses and the element was never optional. No tooling reported the mismatch: the cardinality of a `contains` rule is checked against the base element `BodyStructure.extension`, and an extension definition is evaluated only where an instance of it is present. Instances that omitted the included structure therefore passed validation against 2.0.0 without having been conformant to the extension.
